...
#Useage:
#clip_at_primers.py infile.fasta trimmed.fasta NCTAYGGGRBGCASCAG ATTAGATACCCNNGTAGTCN 15
...
for x in SeqIO.parse(f,'fastq'):
	...
	for primer in p0:
		...
		if m<>[]:
			...
			#print primer,m,q
			l=l+1
			qual[x]=x.letter_annotations["phred_quality"][q:]
			del x.letter_annotations["phred_quality"]
			#print str(x.seq)
			x.seq=x.seq[q:]
			x.letter_annotations["phred_quality"]=qual[x]
			#print str(x.seq),q
			#raw_input()
			q1="y"
			break
